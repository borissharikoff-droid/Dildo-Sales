    ...
    def _setup_google_sheets(self):
        ...
        try:
            ...
            if creds_env:
                ...
                if not os.path.exists(creds_file):
                    error_msg = f"""
❌ Файл с кредами Google API не найден!

📁 Ожидаемые пути:
   - {config.CREDENTIALS_FILE}
   - {os.path.join(config.CREDENTIALS_FOLDER, 'credentials.json')}

📖 Инструкции по получению файла:
   1. Перейдите в Google Cloud Console
   2. Создайте сервисный аккаунт
   3. Скачайте JSON файл с ключами
   4. Переименуйте в 'credentials.json'
   5. Поместите в папку 'credentials/'

Также можно задать переменную окружения GOOGLE_CREDENTIALS_JSON со всем содержимым JSON.
                    """
                ...
                try:
                    ...
                    raise ValueError(f"Файл credentials.json содержит некорректный JSON: {e}")

            # Валидация и нормализация ключа
            required_fields = ['type', 'project_id', 'private_key', 'client_email']
            missing_fields = [field for field in required_fields if field not in creds_data]
            if missing_fields:
                raise ValueError(f"В credentials отсутствуют обязательные поля: {missing_fields}")
            if creds_data.get('type') != 'service_account':
                raise ValueError("Требуется сервисный аккаунт (type: 'service_account')")
...
• <code>@менеджер дата время сумма канал</code>

<b>Примеры:</b>
• <code>@maxim 12 декабря 1111 1489usdt BusinessChannel</code>
...
📺 <b>Канал:</b> {parsed_data['channel']}
            ...
            self.bot.send_message(
                ...
                "<code>@менеджер дата время сумма канал</code>\n\n"
                "Примеры:\n"
                "• <code>@maxim 12 декабря 11:11 1489usdt BusinessChannel</code>\n"
                "• <code>@anna 14.05 11:11 500р каналбизнес</code>",
                parse_mode='HTML'
            )
